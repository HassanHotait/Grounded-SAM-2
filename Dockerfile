...
RUN python -m pip install --upgrade pip setuptools wheel numpy \
    opencv-python transformers supervision pycocotools addict yapf timm nvgpu einops torchserve torch-model-archiver
...
RUN mkdir -p /root/.cache/huggingface/hub/models--microsoft--Florence-2-large
COPY models/Florence-2-large/* /root/.cache/huggingface/hub/models--microsoft--Florence-2-large/
